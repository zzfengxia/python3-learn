...
import re
from datacollect.util import dateu as du
from datacollect.util.formula import MA
import os
from datacollect.util.conns import get_apis, close_apis
from datacollect.fundamental import get_stock_basics
...
def get_realtime_quotes(symbols=None):
    ...
    request = Request(ct.LIVE_DATA_URL % (ct.P_TYPE['http'], ct.DOMAINS['sinahq'], _random(), symbols_list), headers={
        'host': 'hq.sinajs.cn',
...
def quotes(symbols, conn=None, asset='E', market=[], retry_count=3):
    ...
    for _ in range(retry_count):
        try:
            ...
            api, xapi = conn
            data = pd.DataFrame()
            if isinstance(symbols, list) or isinstance(symbols, set) or isinstance(symbols, tuple) or isinstance(
                    symbols, pd.Series):
                for code in symbols:
            ...
            if asset in ['E', 'INDEX']:
                data = data.drop(
                    ['market', 'active1', 'active2', 'reversed_bytes0', 'reversed_bytes1', 'reversed_bytes2',
                     'reversed_bytes3',
                     'reversed_bytes4',
                     'reversed_bytes5',
                     'reversed_bytes6',
...
def reset_instrument(xapi=None):
    """
            重新设置本地证券列表
    """
    import datacollect.util.conns as cs
